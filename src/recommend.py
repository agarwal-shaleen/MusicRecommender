...
def recommend_songs(input_lyrics, top_n=5):
    logging.info("🎵 Recommending songs : ", )
    ...
    #using cleaned lyrics from dataset
    corpus = df['cleaned_text'].fillna(" ").tolist()


    #adding input lyrics at the top
    corpus.insert(0, input_lyrics)
    ...
    #Returning the top recommended songs
    return df.iloc[top_indices][['artist', 'song', 'cleaned_text']]
